          ...
          >
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-gray-900">{item.kategori}</span>
                <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusColors[item.status]}`}>
                  {item.status}
                </span>
              </div>
              <p className="mt-1 line-clamp-2 text-sm text-gray-600">{item.deskripsi}</p>
              <p className="mt-1 text-xs text-gray-400">{item.warga?.nama} &middot; {new Date(item.created_at).toLocaleDateString('id-ID')}</p>
            </div>
            {item.foto?.length > 0 && (
              <img src={`http://localhost:8000/storage/${item.foto[0]}`} alt="" className="ml-4 h-16 w-16 rounded object-cover" />
            )}
          </div>
        ))}
        {list.length === 0 && (
          <div className="py-12 text-center text-gray-400">Belum ada pengaduan</div>
        )}
      </div>
      {meta?.last_page > 1 && (
